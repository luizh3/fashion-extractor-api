...
async def analyze_complete(file: UploadFile = File(...)):
    ...
    """
    logger.info("=" * 50)
    logger.info("📥 INÍCIO - Análise Completa")
    logger.info("=" * 50)
    
    try:
        # Log detalhado do arquivo recebido
        logger.info(f"📋 Informações do arquivo:")
        ...
        logger.info("✅ Tipo de arquivo válido")
        ...
        logger.info(f"   Modo após conversão: {image.mode}")
...
async def analyze_complete_base64(request_data: Dict):
    ...
    try:
        ...
        logger.error(f"❌ Erro geral na análise base64: {e}")
        raise HTTPException(status_code=500, detail=f"Erro ao processar imagem: {str(e)}") 